...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options) {
  ...
  PetscCall(PetscOptionsBool("-closure_tensor", "Use DMPlexSetClosurePermutationTensor()", "ex49.c", options->closure_tensor,
                             &options->closure_tensor, NULL));
  PetscCall(PetscOptionsBool("-project_solution", "Project solution onto mesh", "ex49.c", options->project_solution, &options->project_solution,
                             NULL));
  PetscCall(PetscOptionsBool("-set_partition_balance", "Set partition balance", "DMPlexSetPartitionBalance", options->part_balance,
                             &options->part_balance, NULL));
  ...
  PetscCall(PetscPrintf(comm, "Partition:                             (min,max,median,max/median)\n"));
  {
    ...
    if (comm_size != 1) {
      ...
      if (!rank) {
        ...
        PetscCall(PetscPrintf(comm,
                              "  Ghost Interface %" PetscInt_FMT "-DoF nodes        : %" PetscInt_FMT ", %" PetscInt_FMT ", %" PetscInt_FMT ", %f\n",
